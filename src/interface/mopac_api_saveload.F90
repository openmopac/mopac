...
submodule (mopac_api) mopac_api_saveload
  implicit none

contains

  ! save MOPAC density matrices
  subroutine mopac_save(state, status)
    use molkst_C, only: uhf
    use Common_arrays_C, only: pa, pb
  ...
  subroutine mopac_load(state, status)
    use molkst_C, only: uhf, mpack
    use Common_arrays_C, only: pa, pb
    ...
    if(state%save_state) then
      ...
      ! TO DO: initial guess for density matrices
    end if
  end subroutine mopac_load

  ! save MOZYME density matrix
  subroutine mozyme_save(state, status)
  ...
  subroutine mozyme_load(state, status)
    ...
    if(state%save_state) then
      ...
      ! TO DO: initial guess for density matrix
    end if
