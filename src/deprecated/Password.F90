subroutine password()
  ...
  logical :: exists, new, site_ok, opend, is_windows
...
'++++'/
  ...
  if (i /= 0) then
    ...
    i = len_trim(mopac_pw)
    if (is_windows) then  !  Is a Windows system
      ...
      if (exists) then
        line1 = trim(mopac_pw)//"password for MOPAC2016"
        mopac_pw = trim(line1)
      else
        write(screen,*)" Environmental variable ""MOPAC_LICENSE"" exists, but its value"
      ...
      if (exists) then
        line1 = trim(mopac_pw)//"password_for_mopac2016"
        mopac_pw = trim(line1)
      else
